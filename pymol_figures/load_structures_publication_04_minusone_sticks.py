...
def openfromfile(filelist):
  ...
  for i,tempfilename in enumerate(filelist):
   ...
   for line in lines[:(min(maxload,len(lines)))]:
      ...
      argname_k289 = curprot + " and resi 289 "
      minusone='minusone'
      cmd.create(minusone,argname_P + ' and ( resi 2 )')
      cmd.show_as('sticks',minusone)
      cmd.show('surface',minusone)
      cmd.set('transparency',0.3,minusone)
      cmd.color('aquamarine',minusone)
      cmd.util.cnc(minusone)
      cmd.show_as('sticks', curprot+ ' and (resi 288-291 or resi 257)')#284-294)')
      cmd.color('lightpink', curprot+ ' and (resi 288-291 or resi 257)')#284-294)')
      cmd.util.cnc(curprot+ ' and (resi 288-291 or resi 257)')#284-294)')
      cmd.hide("sticks","hydrogens")
      cmd.color('aquamarine',minusone+ ' and hydrogens')
