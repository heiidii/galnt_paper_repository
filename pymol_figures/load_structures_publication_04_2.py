...
def openfromfile(filelist):
  ...
  for i,tempfilename in enumerate(filelist):
   ...
   for line in lines[:(min(maxload,len(lines)))]:
      ...
      cmd.show("sticks",argname_k289 + ' and not bb.')
      cmd.color("lightpink",argname_k289)
      cmd.set('transparency',0.3,argname_k289)
      cmd.util.cnc(argname_k289)
      cmd.util.cnc(argname_P + ' and ( resi 4 )')
      cmd.color('lightpink',argname_k289+' and hydrogens')
